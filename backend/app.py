...
def normalize_language(language: Optional[str]) -> str:
    """Normalize language parameter to supported codes"""
...
async def chat(req: ChatRequest, request: Request):
    ...
    try:
        ...
        if any(word in message_lower for word in ["help", "emergency", "madad", "मदद", "bachao", "बचाओ"]):
            ...
            }.get(normalize_language(req.language), "🚨 EMERGENCY: Call NDRF immediately: 011-26107953 or Disaster Helpline: 1078.")
        ...
        if ai_response is None:
            ...
            lang = normalize_language(req.language) if normalize_language(req.language) in fallback_responses else "en"
...
# =====================================================
# API CHAT ENDPOINT (ALTERNATIVE - /api/chat)
# =====================================================

@app.post("/api/chat", response_model=ChatResponse)
async def chat_endpoint(request: Request, chat_request: ChatRequest):
    """Alternative chat endpoint under /api/ path"""
    rate_limiter(request, "chat", max_requests=CHAT_RATE_LIMIT_MAX)
...
def delhi_zones(limit: int = 30):
    """Route that uses the helper function"""
    if delhi_model is None:
